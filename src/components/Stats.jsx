...
export default function Stats() {
  return (
    <sectionsection className="bg-premium-sage mx-6 md:mx-12 lg:mx-24 rounded-4xl py-14 px-8 my-4 text-premium-almond">
      ...
      <<divdiv className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
        {stats.map(({ value, label }) => (
          <<divdiv key={label} className="text-center">
            <<pp className="font-display text-5xl font-bold text-premium-almond">{value}</p>
            <<pp className="text-premium-almond/80 mt-2 text-sm">{label}</p>
          </div>
        ))}
      </div>
    </section>
